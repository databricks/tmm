...
-- MAGIC make sure to update the location for Auto Loader (cloud_files) in the three SQL statement below:
-- MAGIC 1. replace `/demos/dlt/loans/raw_transactions` using your ID from the LabGuide notebook, e.g  `/demos/dlt/loans/412602/raw_transactions` assuming your user_id is 412602. Make sure to use to right user_id!
-- MAGIC 2. same for `/demos/dlt/loans/ref_accounting_treatment` replace with `/demos/dlt/loans/412602/ref_accounting_treatment`
-- MAGIC 3. same for `/demos/dlt/loans/historical_loans` replace with `/demos/dlt/loans/412602/historical_loans`
-- MAGIC
...
AS SELECT * FROM cloud_files('/demos/dlt/loans/412602/raw_transactions', 'json', map("cloudFiles.inferColumnTypes", "true"))
...
AS SELECT * FROM delta.`/demos/dlt/loans/412602/ref_accounting_treatment`
...
AS SELECT * FROM cloud_files('/demos/dlt/loans/412602/historical_loans', 'csv', map("cloudFiles.inferColumnTypes", "true"))
