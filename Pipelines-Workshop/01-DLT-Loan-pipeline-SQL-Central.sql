...
-- MAGIC
-- MAGIC * `/demos/dlt/loans/central/raw_transactions` (loans - streaming data)
-- MAGIC * `/demos/dlt/loans/central/ref_accounting_treatment` (reference table, mostly static)
-- MAGIC * `/demos/dlt/loans/central/historical_loans` (loan from legacy system, new data added every week)
-- MAGIC
...
AS SELECT * FROM cloud_files('/Volumes/demo/loans/raw_transactions', 'json', map("cloudFiles.inferColumnTypes", "true"))
...
AS SELECT * FROM delta.`/Volumes/demo/loans/ref_accounting`
...
AS SELECT * FROM cloud_files('/Volumes/demo/loans/historical_loans', 'csv', map("cloudFiles.inferColumnTypes", "true"))
