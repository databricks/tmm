# 8. How do I wrap up and take this tutorial further?
...
![Data flow of the tutorial: the OpenSky Marketplace listing (696M rows, 54,093 aircraft, one UTC day) attaches as the read-only Unity Catalog table marketplace.opensky.state_vectors; Genie Code and Genie Agents run EDA and exploration against it; Genie Code generates a Spark Declarative Pipeline (bronze → silver with EDA-based expectations → gold_americas / gold_emea / gold_apac → gold_analytics_regional_summary) and a Lakeflow Job that runs it hourly with a follow-on notebook; Genie ZeroOps (coming) adds agentic operations on top; a Databricks App coded with Codex, Claude Code, or Genie Code serves gold_apac through a SQL warehouse; and OpenSharing delivers the same share to a local pandas client, all governed by Unity Catalog.](assets/80-wrapup-flow.png)
