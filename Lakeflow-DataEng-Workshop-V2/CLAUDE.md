...
- **Lab 3 — Zerobus direct ingest (live instructor demo; attendees may follow along).**
  ...
  record into the shared Delta table `ops_data.zerobus.measurements` via the **official
  ...
  from the UC config table `ops_data.zerobus.config` (single row, columns: `client_id`,
...
- **Two fixed catalogs** (do not replace with placeholders):
  - `de_workshop` — **attendee** catalog. Per-attendee schema `de_workshop.<user>` (exists; setup notebook never creates/mutates the per-user schemas). Lab 1, 2, 4 pipeline outputs and the Lab 5 Gourmet Pipeline retarget all write here.
  - `ops_data` — **shared ops** catalog. Holds shared assets only:
    - Shared landing volume `/Volumes/ops_data/shared/landing/booking_fraud_flags/` — JSON fraud markers keyed by `booking_id` (Lab 2 source).
    - Zerobus target table `ops_data.zerobus.measurements` and config table `ops_data.zerobus.config` (Lab 3).
- The split keeps attendee-owned data and operationally-managed shared assets cleanly governed: blanket grants on `ops_data` don't leak to attendee schemas, and per-attendee permission cleanup can't touch the shared assets.
...
**Part A — Lab 2 shared assets (in `ops_data`):**
1. Create catalog `de_workshop` (attendee catalog — pre-provisioned in Vocareum, no-op there) and catalog `ops_data` (shared ops catalog — always created here, idempotent).
2. Create schema `ops_data.shared` if it does not exist.
3. Create volume `ops_data.shared.landing` if it does not exist.
4. Create folder `booking_fraud_flags/` in the volume and seed JSON fraud markers
   for **3%** of distinct `booking_id`s from `samples.wanderbricks.booking_updates`.
5. **Grant `USE CATALOG` on `ops_data` and `USE_SCHEMA` on `ops_data.shared` to group `account users`**
   (so attendees can traverse to the volume).
6. **Grant `READ_VOLUME` on `ops_data.shared.landing` to group `account users`**
...
- **B0. Storage preflight.** Create schema `ops_data.zerobus`, then check via the SDK
  ...
  notebooks later. Preflight runs against `ops_data` only (not the attendee catalog).
- **B1.** Create the managed Delta table `ops_data.zerobus.measurements` with exactly
...
- **B4.** Grant the SP: `USE CATALOG` on `ops_data`, `USE SCHEMA` on `ops_data.zerobus`,
  `MODIFY + SELECT` on the data table. Nothing broader — and notably nothing on
  `de_workshop`, which bounds the blast radius even further than before the catalog split.
...
- **B5.** Create the config table `ops_data.zerobus.config` (single row, columns:
  `client_id`, `client_secret`, `workspace_url`, `workspace_id`, `zerobus_endpoint`)
  and grant `USE SCHEMA` on `ops_data.zerobus` plus `SELECT` on both `config` and
  `measurements` to `account users` (with fallback to workspace `users`). The
  `SELECT` on `measurements` lets attendees run the verification `%sql` cell in
  `send_temperature.py`. Cleartext storage of `client_secret` is intentional: every
  attendee needs the secret to authenticate the SDK anyway, and the SP's UC grants
  are tightly bounded — the table simplifies the read path at the cost of
  secret-scope redaction. Attendees read via `spark.table("ops_data.zerobus.config").first()`.
