...
- **Lab 3 — CI/CD via Declarative Automation Bundles with AI Functions.** Deploy a multi-resource bundle (pipeline + dashboard) to the workspace, with AI Functions for in-pipeline LLM calls.
- **Lab 4 — Zerobus Ingest with IoT data** *(live instructor demo; attendees may follow along).* Direct ingest into Delta tables via the Zerobus REST API, with scoped OAuth and secret-scoped credentials. Reference file in [`lab4/`](./lab4/).
- **Lab 5 — Spark Declarative Pipelines with RTM** *(optional / take-home).* Continuous, serverless Real-Time Mode pipeline for sub-second end-to-end latency. Reference bundle in [`lab5/`](./lab5/).
- **Lab 6 — Iceberg side-quest** *(optional / take-home).* Managed Iceberg tables on Unity Catalog, read back from PyIceberg via the UC Iceberg REST Catalog. Reference files in [`lab6/`](./lab6/).
